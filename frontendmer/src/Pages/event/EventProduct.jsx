...
import { addEvent } from "../../State/cart_actions";
  ...
  return (
    <div className="col-lg-3 col-md-4 col-sm-6 col-12 mb-4" key={item.id}>
      ...
      <div className="custom-card d-flex flex-column h-100">
        <Link to={`/event-details/${item._id}`} className="custom-card-image-container">
          <img
            src={item.image[0]}
            className="h-fit w-fit custom-card-image object-fit-contain"
            alt={item.name}
          />
        </Link>
        <div className="custom-card-body d-flex flex-column justify-content-between">
          <div className="d-flex flex-column flex-grow-1">
            <div className="d-flex justify-content-between align-items-center">
              <h5 className="custom-card-title line-through mb-0">Rs.{item?.originalPrice}</h5>
              <h5 className="custom-card-title mb-0">Rs.{item.price}</h5>
            </div>
