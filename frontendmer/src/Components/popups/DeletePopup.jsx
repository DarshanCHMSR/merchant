...
const DeletePopup = ({ show, handleClose, id }) => {
  ...
  const cordinates = {
    longitude: auth?.user?.location?.longitude,
    latitude: auth?.user?.location?.latitude,
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // console.log("The response of cancel order is ", res.data);
      if (res.data.success) {
        const data = {
          name: auth?.user?.name,
        ...
        const sendmail = await sendCancellationRequest(data);
