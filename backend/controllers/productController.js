...
export const fetchAllProducts = async (req, res) => {
  ...
  export const getUserTotalProducts = async (req, res) => {  
    try {
      ...
      const totalProductswaiting = await Product.find({user: user_id }).countDocuments({ status:0 });
      const totalProductsapproved = await Product.find({user: user_id }).countDocuments({ status:1 });


      res.status(200).json({
        success: true,
        totalProducts,
        totalProductswaiting,
